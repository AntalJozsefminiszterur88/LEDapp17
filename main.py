...
async def attempt_auto_connect(app_instance):
    ...
    try:
        ...
        if not future:
            log_event(
                "Hiba: Nem sikerült elindítani az automatikus csatlakozási taskot."
            )
        ...
        log_event(
            f"_initial_connection_attempted flag beállítva: True (Csatlakozás a háttérben fut/futott)."
        )
...
if __name__ == "__main__":
    ...
    if os.path.exists(icon_path):
        ...
        if os.path.exists(frozen_icon_path):
            ...
            print(
                f"Figyelmeztetés: Az ikonfájl nem található a fagyasztott helyen sem: {frozen_icon_path}"
            )
    else:
    ...
    if start_hidden_arg:
        ...
        if config_manager.get_setting("auto_connect_on_startup"):
            ...
            main_window._initial_connection_attempted = (
                True  # Jelöljük, hogy nem kell várni
            )

    else:
        log_event("Normál indítás.")
        main_window._initial_connection_attempted = (
            True  # Normál indításnál nincs auto-connect kísérlet
        )
        main_window.show()
