...
def remove_custom_color(name: str):
    ...
    removed_entries = [c for c in CUSTOM_COLORS if c[0] == name]
    if not removed_entries:
        return False

    CUSTOM_COLORS[:] = [c for c in CUSTOM_COLORS if c[0] != name]
    COLORS[:] = [c for c in COLORS if c[0] != name or c not in removed_entries]
    save_custom_colors(CUSTOM_COLORS)
    return True
