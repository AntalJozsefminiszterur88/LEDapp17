...
async def quick_find_by_address(address):
    ...
    try:
        dev = await BleakScanner.find_device_by_address(
            address, timeout=FAST_FIND_TIMEOUT
        )
        if dev:
...
async def start_ble_connection_loop(app, stop_event: threading.Event):
    ...
    if not app.selected_device or not app.selected_device[0]:
        log_event(
            "Hiba: Nincs kiválasztott eszköznév a kapcsolattartáshoz. Loop leáll."
        )
        return
    ...
    log_event(
        f"Kapcsolat figyelő indítása: '{original_device_name}' ({current_address})"
    )
    ...
    while True:
        ...
        try:
            ...
            if not current_client or not current_client.is_connected:
                ...
                log_event(
                    f"Kapcsolat ellenőrzés: Nincs kapcsolat '{original_device_name}' ({current_address}). Próba #{connection_attempts + 1}..."
                )
                ...
                if connection_attempts >= MAX_CONNECT_ATTEMPTS:
                    ...
                    if new_address:
                        ...
                        log_event(
                            f"Eszköz nem található keresés után sem. Várakozás ({RESCAN_DELAY}s)..."
                        )
                ...
                try:
                    if hasattr(app, "connection_status_signal"):
                        app.connection_status_signal.emit("connecting")
                    ...
                    if app.ble and app.ble.client:
                        ...
                        app.ble.client = None
                        try:
                            if old_client_ref.is_connected:
                                await old_client_ref.disconnect()
                            log_event("Régi kliens bontva.")
                        except Exception as disconn_err:
                            log_event(
                                f"Figyelmeztetés: Hiba a régi kliens bontásakor: {disconn_err}"
                            )

                    log_event(
                        f"Új BleakClient létrehozása és hozzárendelése: {current_address}..."
                    )
                    client = BleakClient(current_address)
                    app.ble.client = client

                    log_event(
                        f"Csatlakozás megkezdése: {current_address} (timeout={CONNECT_TIMEOUT}s)..."
                    )
                    await client.connect(timeout=CONNECT_TIMEOUT)

                    if hasattr(app, "connection_status_signal"):
                        app.connection_status_signal.emit("connected")
                    app.connection_status = "connected"
                    log_event(
                        f"Sikeresen csatlakozva: '{original_device_name}' ({current_address})"
                    )
                    last_ping_time = time.time()
                    connection_attempts = 0
                    ...
                    log_event(
                        f"Kapcsolódási hiba #{connection_attempts + 1} ({type(e).__name__}): {e}"
                    )
                    if hasattr(app, "connection_status_signal"):
                        app.connection_status_signal.emit("disconnected")
                    app.connection_status = "disconnected"
                    ...
                    if client_that_failed:
                        log_event(
                            "Kapcsolódási hiba utáni kliens bontás kísérlete (app.ble.client)..."
                        )
                        app.ble.client = None
                        try:
                            await client_that_failed.disconnect()
                        except Exception as disconn_err_on_fail:
                            log_event(
                                f"Figyelmeztetés: Hiba a kliens bontásakor hiba után: {disconn_err_on_fail}"
                            )
                    ...
                    log_event(
                        f"Általános hiba a kapcsolatban #{connection_attempts + 1}: {e}"
                    )
                    ...
                    if stop_event.is_set():
                        log_event(
                            "Stop event észlelve általános hiba után, loop leáll."
                        )
                        break
                    # ********************************************
                    await asyncio.sleep(RECONNECT_DELAY)
            ...
            # --- Ha Csatlakozva van: Keep-Alive Ping ---
                ...
                if app.connection_status != "connected":
                    log_event(
                        "Kliens csatlakozva, de app státusz nem 'connected'. Státusz frissítése."
                    )
                    if hasattr(app, "connection_status_signal"):
                        app.connection_status_signal.emit("connected")
                    app.connection_status = "connected"
                ...
                now = time.time()
                last_input_time = (
                    app.last_user_input if hasattr(app, "last_user_input") else now
                )
                ...
                should_ping = elapsed_since_last_ping >= PING_INTERVAL or (
                    elapsed_since_input >= INACTIVITY_PING_THRESHOLD
                    and elapsed_since_last_ping >= INACTIVITY_PING_THRESHOLD
                )

                if should_ping:
                    try:
                        if current_client and current_client.is_connected:
                            await current_client.write_gatt_char(
                                CHARACTERISTIC_UUID,
                                bytes.fromhex(KEEP_ALIVE_COMMAND),
                                response=False,
                            )
                            last_ping_time = time.time()
                            connection_attempts = 0
                            ...
                            log_event(
                                "Ping kihagyva, a kliens már nem csatlakozik (pingelés előtt ellenőrizve)."
                            )
                        ...
                        if ping_failed_client:
                            ...
                            app.ble.client = None
                            try:
                                await ping_failed_client.disconnect()
                            except Exception as disconn_err_ping:
                                log_event(
                                    f"Hiba a kliens bontásakor ping hiba után: {disconn_err_ping}"
                                )
                        connection_attempts += 1
                        # *** STOP EVENT ELLENŐRZÉSE HIBA ESETÉN IS ***
                        if stop_event.is_set():
                        ...
                        if stop_event.is_set():
                            log_event(
                                "Stop event észlelve általános ping hiba után, loop leáll."
                            )
                            break
                        # ********************************************
            ...
            if stop_event.is_set():
                log_event(
                    "Stop event észlelve (ciklus végén), reconnect loop leállítása..."
                )
                break
            # ************************************************
            ...
            log_event(
                "A start_ble_connection_loop fő ciklusa megszakadt (CancelledError). Loop leáll."
            )
            break
    ...
    # Loop végén cleanup
    log_event(
        "start_ble_connection_loop vége (while ciklusból kilépve), utolsó cleanup..."
    )
    ...
    if final_client and final_client.is_connected:
        try:
            ...
            try:
                ...
                try:
                    asyncio.run(final_client.disconnect())
                except Exception as e_run_disconn:
                    log_event(
                        f"Hiba az új loopban futtatott disconnect során: {e_run_disconn}"
                    )
