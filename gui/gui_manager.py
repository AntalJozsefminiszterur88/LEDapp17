...
# Importok
try:
    from core.reconnect_handler import log_event, start_ble_connection_loop
    ...
    log_event(
        f"HIBA: Nem sikerült importálni a szükséges modulokat a gui_manager.py-ban: {e}"
    )
...
class GuiManager:
                ...
                if widget is not None:
                    if hasattr(widget, "stop_timers") and callable(widget.stop_timers):
                        try:
                            ...
                            log_event(
                                f"Hiba a widget stop_timers hívásakor (_clear_layout): {e}"
                            )
                    widget.deleteLater()
                else:
    ...
    def clear_window_content(self):
        ...
        if isinstance(current_widget, GUI2_Widget):
            log_event(
                "clear_window_content: GUI2 volt aktív, reconnect loop stop jelzés..."
            )
        ...
        if current_widget:
            log_event(
                f"clear_window_content: Aktuális widget törlése: {current_widget.objectName()}"
            )
            if hasattr(current_widget, "stop_timers") and callable(
                ...
                try:
                    ...
                    log_event(
                        f"Hiba a {current_widget.objectName()} stop_timers hívásakor: {e}"
                    )
            ...
            log_event(
                "clear_window_content: Nem volt aktuális widget referencia, layout ürítése fallbackként."
            )
    ...
    def load_gui2(self):
        ...
        self.app.update_connection_status_gui(
            self.app.connection_status
        )  # GUI állapot frissítése
        self.center_window()
        ...
        if self.reconnect_thread is None or not self.reconnect_thread.is_alive():
            ...
            if hasattr(self.app, "_stop_reconnect_event"):
                ...
                log_event(
                    "HIBA: Nincs _stop_reconnect_event az app példányon GUI2 töltésekor!"
                )
