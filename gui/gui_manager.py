...
class GuiManager:
    ...
    def load_gui2(self):
        ...
        if self.reconnect_thread is None or not self.reconnect_thread.is_alive():
            ...
            self.reconnect_thread = threading.Thread(
                ...
                args=(self.app, self.app._stop_reconnect_event), # Átadjuk az appot és az eventet
                daemon=True
            )
    ...
    def _run_reconnect_loop_target(app_instance, stop_event):
         ...
         try:
              ...
              if loop.is_running():
                  try:
                      if sys.version_info >= (3, 7): loop.run_until_complete(loop.shutdown_asyncgens())
                      loop.call_soon_threadsafe(loop.stop)
                  except RuntimeError as e: log_event(f"RuntimeError during reconnect loop stop: {e}")
