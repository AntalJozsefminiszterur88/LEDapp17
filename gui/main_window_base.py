...
from PySide6.QtWidgets import (
    QApplication,  # QSystemTrayIcon hozzáadva
    QFrame,
    QLabel,
    QListWidget,
    QMainWindow,
...
try:
    from config import COLORS, CONFIG_FILE, DAYS
    ...
    def log_event(msg):
        print(f"[LOG - Dummy BaseWindow]: {msg}")
...
try:
    ...
    try:
        ...
        log_event(
            "Figyelmeztetés: 'Europe/Budapest' időzóna sem található. UTC használata."
        )
        LOCAL_TZ = pytz.utc
...
class LEDApp_BaseWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.selected_device = (
            ...
            if config_manager.get_setting("last_device_address")
            else None
        )
        ...
        async def do_disconnect():
            try:
                if self.ble and self.ble.client:
                    ...
                    log_event(
                        "BLE disconnect nem szükséges (nincs kliens vagy már bontva)."
                    )
                ...
                QMetaObject.invokeMethod(
                    self, "_load_gui1_slot", Qt.ConnectionType.QueuedConnection
                )

        # Aszinkron disconnect indítása az AsyncHelperrel
        self.async_helper.run_async_task(
            do_disconnect(), None, self.command_error_signal
        )
    ...
    def update_connection_status_gui(self, status):
        ...
        if hasattr(self, "tray_icon") and self.tray_icon:
            device_name_str = (
                f": {self.selected_device[0]}"
                if self.selected_device and self.selected_device[0]
                else ""
            )
            if status == "connected":
                tooltip = f"LED-Irányító 2000 (Csatlakozva{device_name_str})"
            elif status == "connecting":
                tooltip = f"LED-Irányító 2000 (Csatlakozás...{device_name_str})"
            else:
                tooltip = "LED-Irányító 2000 (Nincs kapcsolat)"
        ...
        if isinstance(current_widget, GUI2_Widget):
            ...
            if label and label.isVisible():
                if status == "connected":
                    text, color = "Állapot: Csatlakoztatva", "lime"
                elif status == "connecting":
                    text, color = "Állapot: Csatlakozás...", "#FFA500"
                else:
                    text, color = "Állapot: Nincs kapcsolat", "#FF6B6B"
                label.setText(text)
                label.setStyleSheet(
                    f"QLabel {{ color: {color}; font-weight: bold; background-color: transparent; }}"
                )
            ...
            if progress_label and progress_label.isVisible():
                if status == "connecting":
                    progress_label.setText("Csatlakozás...")
                elif status == "disconnected":
                    progress_label.setText("Kapcsolat bontva.")
                elif status == "connected":
                    progress_label.setText("Csatlakozva")  # GUI1-en is jelezzük

    @Slot(object)
    def _handle_scan_results(self, devices):
        log_event(
            f"_handle_scan_results SLOT triggered in GUI thread. Received data type: {type(devices)}, Value: {devices}"
        )
        current_widget = self._current_gui_widget
        if isinstance(current_widget, GUI1_Widget):
            if isinstance(devices, list):
                ...
                log_event(
                    f"Hiba: Váratlan típus érkezett a scan eredményeként: {type(devices)}"
                )
                current_widget.on_scan_error(
                    "Belső hiba: érvénytelen keresési eredmény."
                )
    ...
    def _handle_scan_error(self, error_message):
        log_event(
            f"_handle_scan_error SLOT triggered in GUI thread. Error: {error_message}"
        )
        current_widget = self._current_gui_widget
        if isinstance(current_widget, GUI1_Widget):
            ...
            log_event(
                f"Figyelmeztetés: Scan hiba ({error_message}), de nem a GUI1 aktív."
            )

    @Slot(bool)
    def _handle_connect_results(self, success):
        log_event(
            f"_handle_connect_results SLOT triggered in GUI thread. Success: {success}"
        )
        current_widget = self._current_gui_widget
        self._initial_connection_attempted = (
            True  # Jelöljük, hogy a kezdeti próbálkozás megtörtént
        )
        ...
        if success:
            ...
            if self.selected_device:
                config_manager.set_setting(
                    "last_device_address", self.selected_device[1]
                )
                config_manager.set_setting("last_device_name", self.selected_device[0])
                log_event(
                    f"Utolsó eszköz elmentve: {self.selected_device[0]} ({self.selected_device[1]})"
                )
            else:
                log_event(
                    "Figyelmeztetés: Sikeres csatlakozás, de self.selected_device üres."
                )
    ...
    def _handle_connect_error(self, error_message):
        log_event(
            f"_handle_connect_error SLOT triggered in GUI thread. Error: {error_message}"
        )
        current_widget = self._current_gui_widget
        self.connected = False  # Biztosan nem vagyunk csatlakozva
        self._initial_connection_attempted = (
            True  # Jelöljük, hogy a kezdeti próbálkozás megtörtént (de sikertelen volt)
        )
        ...
        if isinstance(current_widget, GUI1_Widget):
            ...
            log_event(
                f"Figyelmeztetés: Connect hiba ({error_message}), de nem a GUI1 aktív."
            )
            self.update_connection_status_gui("disconnected")
            if hasattr(self, "statusBar") and callable(self.statusBar):
                self.statusBar().showMessage(
                    f"Kapcsolódási hiba: {error_message}", 5000
                )
    ...
    def _handle_command_error(self, error_message):
        log_event(
            f"_handle_command_error SLOT triggered in GUI thread. Error: {error_message}"
        )
        if hasattr(self, "statusBar") and callable(self.statusBar):
