...
class GUI1_Widget(QWidget):
    def __init__(self, main_app, parent=None):
        ...
        self.device_listbox.setSelectionMode(
        ...
        self.disconnect_button.setSizePolicy(
            QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Fixed
        )
        ...
        self.goto_gui2_button.setSizePolicy(
            QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed
        )
        self.goto_gui2_button.clicked.connect(
            self.main_app.gui_manager.load_gui2
        )  # GuiManageren keresztül
    ...
    def update_button_states(self):
        ...
        is_scanning_or_connecting = (
            self.progress_bar.minimum() == 0 and self.progress_bar.maximum() == 0
        )
        ...
        self.connect_button.setEnabled(
            has_devices and not connected and not is_scanning_or_connecting
        )
    ...
    def on_scan_error(self, error_message):
        ...
        QMessageBox.critical(
            self, "Keresési Hiba", f"Hiba a keresés során:\n{error_message}"
        )
        ...
        if not selected_items:
            if not self.main_app.devices:
                QMessageBox.information(
                    self, "Nincs Eszköz", "Először keress eszközöket."
                )
                return
            QMessageBox.warning(
                self, "Nincs kiválasztva", "Kérlek válassz ki egy eszközt a listából."
            )
            return
