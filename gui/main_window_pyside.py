...
from PySide6.QtCore import (
    Q_ARG,  # QMetaObject és Q_ARG hozzáadva
    QMetaObject,
    Qt,
    QTimer,
    Slot,
...
class LEDApp_PySide(LEDApp_BaseWindow):
    def __init__(
        self, start_hidden=False, parent=None
    ):  # start_hidden paraméter hozzáadva
        super().__init__(parent)
        self._force_quit = False
        self._start_hidden = start_hidden  # Indítási állapot tárolása
        self._initial_gui_loaded = (
            False  # Segédfalg, hogy tudjuk, betöltöttük-e már a GUI-t
        )
        ...
        app_icon = (
            QApplication.instance().windowIcon() if QApplication.instance() else QIcon()
        )
        if app_icon.isNull():
            ...
            if os.path.exists(icon_path):
                ...
                if os.path.exists(frozen_icon_path):
                    ...
                    log_event(
                        f"Alkalmazás ikont a fagyasztott helyről '{frozen_icon_path}'-ból töltöttük be."
                    )
                else:
                    log_event(
                        f"Figyelmeztetés: Az ikonfájl ('{icon_path}') nem található sem relatívan, sem a fagyasztott helyen."
                    )
            else:
        ...
        if not app_icon.isNull():
            ...
            log_event(
                "Figyelmeztetés: Tálca ikon nem hozható létre, ikon nem elérhető."
            )
            if self._start_hidden:
                log_event(
                    "HIBA: Rejtett indítás kérése tálca ikon nélkül nem lehetséges!"
                )
    ...
    def load_initial_gui(self):
        ...
        if self._initial_gui_loaded:
            return  # Ne töltse be újra, ha már megtörtént
        ...
        if (
            ...
            and self._initial_connection_attempted
        ):
        ...
        # akkor várjunk, és ne töltsük be még a GUI1-et sem, ha rejtve indultunk.
            ...
            log_event(
                "Kezdeti GUI betöltése felfüggesztve, amíg az auto-connect befejeződik."
            )
            ...
            log_event(
                "Nincs kapcsolat, vagy nincs eszköz, vagy auto-connect sikertelen. GUI1 betöltése..."
            )
