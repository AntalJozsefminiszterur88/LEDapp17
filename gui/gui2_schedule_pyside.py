...
class GUI2_Widget(QWidget):
    def __init__(self, main_app, parent=None):
        ...
        def load_profile_into_gui(name):
            ...
            for day, widgets in self.schedule_widgets.items():
                ...
                if "on_time2" in widgets:
                    widgets["on_time2"].setCurrentText(data.get("on_time2", ""))
                if "off_time2" in widgets:
                    widgets["off_time2"].setCurrentText(data.get("off_time2", ""))
        ...
        for col in range(2, 6):
            table_layout.setColumnStretch(col, 0)
        table_layout.setColumnStretch(7, 0)
        table_layout.setColumnStretch(9, 0)
        headers = ["Nap", "Szín", "Fel", "Le", "Fel2", "Le2", "Napkelte", "+/-", "Napnyugta", "+/-"]
        ...
        for i, day_hu in enumerate(DAYS):
            ...
            on_time2_cb = QComboBox(); on_time2_cb.addItems(time_values); on_time2_cb.setCurrentText(schedule_data.get("on_time2", "")); on_time2_cb.setEditable(True); on_time2_cb.setFixedWidth(70)
            on_time2_cb.currentTextChanged.connect(self.mark_unsaved)
            table_layout.addWidget(on_time2_cb, row, 4, Qt.AlignmentFlag.AlignCenter); day_widgets["on_time2"] = on_time2_cb; self.time_comboboxes.append(on_time2_cb)
            off_time2_cb = QComboBox(); off_time2_cb.addItems(time_values); off_time2_cb.setCurrentText(schedule_data.get("off_time2", "")); off_time2_cb.setEditable(True); off_time2_cb.setFixedWidth(70)
            off_time2_cb.currentTextChanged.connect(self.mark_unsaved)
            table_layout.addWidget(off_time2_cb, row, 5, Qt.AlignmentFlag.AlignCenter); day_widgets["off_time2"] = off_time2_cb; self.time_comboboxes.append(off_time2_cb)
            sunrise_cb = QCheckBox(); sunrise_cb.setChecked(schedule_data.get("sunrise", False)); table_layout.addWidget(sunrise_cb, row, 6, Qt.AlignmentFlag.AlignCenter)
            day_widgets["sunrise"] = sunrise_cb
            sunrise_cb.stateChanged.connect(
                partial(self._handle_sunrise_change, idx=i * 2, day=day_hu)
            )
            ...
            table_layout.addWidget(sunrise_offset_entry, row, 7, Qt.AlignmentFlag.AlignCenter); day_widgets["sunrise_offset"] = sunrise_offset_entry
            sunset_cb = QCheckBox(); sunset_cb.setChecked(schedule_data.get("sunset", False)); table_layout.addWidget(sunset_cb, row, 8, Qt.AlignmentFlag.AlignCenter)
            day_widgets["sunset"] = sunset_cb
            sunset_cb.stateChanged.connect(
                partial(self._handle_sunset_change, idx=i * 2 + 1, day=day_hu)
            )
            ...
            table_layout.addWidget(sunset_offset_entry, row, 9, Qt.AlignmentFlag.AlignCenter); day_widgets["sunset_offset"] = sunset_offset_entry
    ...
    def change_profile(self, name: str):
        ...
        if self.unsaved_changes:
            ...
            if clicked == yes_btn:
                ...
                self.unsaved_changes = False
            if clicked in (cancel_btn,) or self.unsaved_changes:
    ...
    def toggle_profile_active(self, state):
        ...
        if desired_active and not logic.validate_profile(prof):
            QMessageBox.warning(
                self,
                "Hiba",
    ...
    def reset_schedule_gui(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            for day, widgets in self.schedule_widgets.items():
                ...
                if "on_time2" in widgets:
                    widgets["on_time2"].setCurrentText("")
                if "off_time2" in widgets:
                    widgets["off_time2"].setCurrentText("")
    ...
    @Slot(int)
    def _handle_sunrise_change(self, state, idx, day):
        """Kezeli a napkelte jelölő állapotváltozását."""
        self.mark_unsaved()
        self.toggle_sun_time(state, idx, day, "sunrise")

    @Slot(int)
    def _handle_sunset_change(self, state, idx, day):
        """Kezeli a napnyugta jelölő állapotváltozását."""
        self.mark_unsaved()
        self.toggle_sun_time(state, idx, day, "sunset")
