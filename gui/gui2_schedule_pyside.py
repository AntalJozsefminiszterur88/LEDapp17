...
from PySide6.QtWidgets import (
    QCheckBox,
    QComboBox,  # QGroupBox eltávolítva
    QFrame,
    QGridLayout,
    QHBoxLayout,
    QInputDialog,
...
try:
    ...
    from core.location_utils import LOCAL_TZ, get_sun_times
    ...
    log_event(
        f"KRITIKUS HIBA: Nem sikerült importálni a szükséges modulokat gui2_schedule_pyside.py-ban: {e}"
    )
    traceback.print_exc()
...
class GUI2_Widget(QWidget):
    def __init__(self, main_app, parent=None):
        ...
        top_left_layout.setAlignment(
            Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignLeft
        )
        device_name = (
            self.main_app.selected_device[0]
            if self.main_app.selected_device
            else "Ismeretlen"
        )
        ...
        sunrise_str = (
            self.main_app.sunrise.strftime("%H:%M") if self.main_app.sunrise else "N/A"
        )
        sunset_str = (
            self.main_app.sunset.strftime("%H:%M") if self.main_app.sunset else "N/A"
        )
        ...
        self.coord_label = QLabel(
            f"Koordináták: {lat:.4f}°É, {lon:.4f}°K | Időzóna: {tz_name}"
        )
        ...
        self.position_status_label.setStyleSheet(
            f"color: {status_color}; background-color: transparent;"
        )
        ...
        if not hasattr(self.main_app, "profiles") or not self.main_app.profiles:
            self.main_app.profiles = {
                "Alap": {"active": True, "schedule": logic.get_default_schedule()}
            }

        self.default_profile_name = list(self.main_app.profiles.keys())[0]
        self.current_profile_name = self.default_profile_name
        self.main_app.schedule = self.main_app.profiles[self.current_profile_name][
            "schedule"
        ]
        ...
        for i, header in enumerate(headers):
            ...
            align = (
                Qt.AlignmentFlag.AlignLeft if i == 0 else Qt.AlignmentFlag.AlignCenter
            )
        ...
        for i, day_hu in enumerate(DAYS):
            ...
            color_cb.setCurrentIndex(
                color_display_names.index(saved_color)
                if saved_color in valid_color_names
                else 0
            )
            ...
            sunrise_offset_entry = QLineEdit(
                str(schedule_data.get("sunrise_offset", 0))
            )
            ...
            table_layout.addWidget(
                sunrise_offset_entry, row, 5, Qt.AlignmentFlag.AlignCenter
            )
            ...
            table_layout.addWidget(
                sunset_offset_entry, row, 7, Qt.AlignmentFlag.AlignCenter
            )
        ...
        main_layout.addWidget(
            table_container, 0, Qt.AlignmentFlag.AlignCenter
        )  # Hozzáadás a fő layout-hoz
        ...
        if (
            not isinstance(config_manager, type)
            or config_manager.__name__ != "DummyManager"
        ):
            try:
                self.startup_checkbox.setChecked(
                    config_manager.get_setting("start_with_windows")
                )
    ...
    def is_schedule_modified(self):
        ...
        for day, widgets in self.schedule_widgets.items():
            try:
                sr_off = (
                    int(widgets["sunrise_offset"].text())
                    if widgets["sunrise_offset"].text()
                    else 0
                )
            except ValueError:
                sr_off = 0
            try:
                ss_off = (
                    int(widgets["sunset_offset"].text())
                    if widgets["sunset_offset"].text()
                    else 0
                )
            except ValueError:
                ss_off = 0
        ...
        original = self.main_app.profiles.get(self.current_profile_name, {}).get(
            "schedule", {}
        )
    ...
    def change_profile(self, name: str):
        ...
        self.profile_active_checkbox.setChecked(
            self.main_app.profiles[name].get("active", True)
        )
    ...
    def delete_profile(self):
        ...
        if self.current_profile_name == getattr(self, "default_profile_name", ""):
            QMessageBox.information(
                self, "Hiba", "Az alapértelmezett profil nem törölhető."
            )
            return
    ...
    def toggle_startup(self, state):
        ...
        log_event(
            f"'Indítás a Windows-zal' checkbox {'bekapcsolva' if is_checked else 'kikapcsolva'}."
        )

        is_dummy_cfg = (
            isinstance(config_manager, type)
            and config_manager.__name__ == "DummyManager"
        )
        is_dummy_reg = (
            isinstance(registry_utils, type)
            and registry_utils.__name__ == "DummyManager"
        )
        ...
        if not success:
            log_event(
                "Registry művelet sikertelen, checkbox és beállítás visszaállítása."
            )
            self.startup_checkbox.blockSignals(True)
            self.startup_checkbox.setChecked(not is_checked)
            self.startup_checkbox.blockSignals(False)
    ...
    def update_time(self):
        ...
        try:
            ...
            self.time_label.setText(
                f"{now.strftime('%Y.%m.%d')} | {magyar_nap} | {now.strftime('%H:%M:%S')}"
            )
        except Exception as e:
