...
class AsyncHelper:
    ...
    def __init__(self, app_instance):
        ...
        self.event_loop_thread = threading.Thread(
            target=self._run_dedicated_asyncio_loop, daemon=True
        )
        ...
        try:
            ...
            try:
                ...
                if all_tasks:
                    ...
                    current_task = (
                        asyncio.current_task(self.loop)
                        ...
                        try:
                            if self.loop.is_running():
                                self.loop.run_until_complete(
                                    asyncio.wait_for(gather_future, timeout=2.0)
                                )
    ...
    def run_async_task(
        self, coro, callback_success_signal=None, callback_error_signal=None
    ):
        """
        ...
        if not self.loop.is_running():
            ...
            if callback_error_signal and isinstance(callback_error_signal, Signal):
                ...
                log_event(
                    f"HIBA: Nem található vagy nem Signal a megadott error callback: {callback_error_signal}"
                )
            return None
        ...
        def done_callback(f):
            try:
                result = f.result()
                log_event(
                    f"AsyncHelper: Task successful. Result type: {type(result)}, Value: {result}"
                )
                if callback_success_signal and isinstance(
                    callback_success_signal, Signal
                ):
                    # Név logolása nélkül
                    log_event(
                        f"AsyncHelper: Emitting success signal with data type {type(result)}..."
                    )
                ...
                if hasattr(e, "dbus_error"):
                    bleak_error_msg = (
                        f" (DBus Error: {getattr(e, 'dbus_error_details', '')})"
                    )
                ...
                )  # Hibakereséshez adjuk hozzá a tracebacket
                if callback_error_signal and isinstance(callback_error_signal, Signal):
                    # Név logolása nélkül
